...
Свежая база: **5 черновиков · 10 опубликованных задач · 5 команд · 5 откликов** при запуске `scripts/demo.py`. Пять опубликованных задач — исходные примеры, ещё пять — учебные кейсы на реальных открытых данных. Рейтинги исходных примеров: **0, 30, 50, 75, 100**; кейсы на открытых данных имеют **90 / 100**: у них нет реального заказчика и контакта для консультаций. Данные синтетические; повторная подготовка сохраняет существующие данные.

| Учебный кейс | Открытый источник |
| --- | --- |
| Экономика Казахстана | [Всемирный банк](https://data.worldbank.org/country/kazakhstan) |
| Планировщик прогулок по погоде Алматы | [Open-Meteo](https://open-meteo.com/en/docs) |
| Карта зарегистрированных землетрясений | [USGS](https://earthquake.usgs.gov/fdsnws/event/1/) |
| Обзор природных событий | [НАСА EONET](https://eonet.gsfc.nasa.gov/docs/v3) |
| Карта инфраструктуры района Алматы | [OpenStreetMap](https://www.openstreetmap.org/export), [лицензия и атрибуция](https://www.openstreetmap.org/copyright) |

Это учебные задания на настоящих источниках, а не заказы перечисленных организаций. В карточках указаны ограничения, результат и критерии проверки. Данные нужно загрузить из источника при выполнении задания; Lovelab не выдаёт выдуманные наблюдения за реальные. Отдельное добавление: `python -m backend.open_data_seed`, повторный запуск не создаёт копии.
