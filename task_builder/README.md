...
`Task` содержит поля из `TEAM_PLAN.md`: `id`, `title`, `topic`, `context`, `need`, `users`, `data`, `constraints`, `expected_result`, `success_criteria`, `contact`, `interaction_format`, `status`, `score`. В API конструктора есть служебное `confirmed_at` для контроля подтверждения. Сейчас задачи хранятся в отдельном SQLite файле `task_builder/tasks.db` (путь настраивается через `TASK_BUILDER_DB_PATH`). При интеграции нужен интерфейс общей БД для создания и обновления задачи, а также вызов пересчёта рейтинга после подтверждения. Схема, рейтинг, каталог и общая навигация принадлежат другим участникам.
