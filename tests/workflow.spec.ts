...
test('business task through publication, proposal, manual selection and progress', async ({ page }) => {
  ...
  await expect(page.getByRole('status').filter({ hasText: 'локальные вопросы' })).toBeVisible();
  ...
  await page.getByLabel('Ссылка на прототип').fill('https://example.com/support');
